...
-   事件处理（[了解更多](https://www.wolai.com/kyFr1KjtBxVBPS79Kzv9Ts#tSp8JzyXmyB7VahC2eYpmV)）
-   异步代码（例如 `setTimeout` 或 `requestAnimationFrame` 回调函数）
-   服务端渲染
-   它自身抛出来的错误（并非它的子组件）

如果一个 class 组件中定义了static getDerivedStateFromProps() 或componentDidCatch()这两个生命周期方法中的任意一个（或两个）时，那么它就变成一个错误边界。当抛出错误后，请使用 `static getDerivedStateFromError()` 渲染备用 UI ，使用 `componentDidCatch()` 打印错误信息。
...
![](https://secure2.wostatic.cn/static/mHUB4ZKg8x1f38T3RX9uSk/image.png?auth_key=1668419835-qHKG5usSNzNwqbbSH8jYd6-0-55ba22501c5bfbc7352efcfcd147d6e4)

你也可以在组件栈追踪中查看文件名和行号，这一功能在 [Create React App](https://github.com/facebookincubator/create-react-app) 项目中默认开启：

![](https://secure2.wostatic.cn/static/5w1TnqGaMXwNkcFcbvQJDV/image.png?auth_key=1668419835-7r8YjJpqoVv1WVEAj3oSRY-0-0676ef36daa8dff73a8ce8ed8d8f64c5)
