...
我们会添加一个ref到这个根 DOM 元素。 在 `componentDidMount` 中，我们能够获取它的引用这样我们就可以把它传递给 jQuery 插件了。
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/qmqeQx?editors=0010)
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/bWgbeE?editors=0010)
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/xdgKOz?editors=0010)

---

# 和其他视图库集成

得益于ReactDOM.render()的灵活性，React 可以被嵌入到其他的应用中。
...
从这起你可开始以把更多的逻辑移动到组件中，并且开始应用更多通用 React 实践。例如，在组件中最好不要依赖 ID 因为同一个组件可能会被渲染多次。相反的，我们会使用 React事件处理并且直接注册 click 处理函数到 React `<button>` 元素：
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/RVKbvW?editors=1010)
...
如下，我们会创建一个名为 `ParagraphView` 的 Backbone 视图。他会重载 Backbone 的 `render()` 函数来渲染一个 React `<Paragraph>` 组件到 Backbone (`this.el`) 提供的 DOM 元素中。这里，同样的，我们将会使用ReactDOM.render()：
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/gWgOYL?editors=0010)

在 `remove` 方法中我们也需要调用 `ReactDOM.unmountComponentAtNode()` 以便在它解除的时候 React 清理组件树相关的事件处理的注册和其他的资源，这点是是很重要的。

当一个组件在 React 树中_从内部_删除的时候，清理工作是自动完成的，但是因为我们现在手动移除整个树，我们必须调用这个方法。

### 和 Model 层集成

虽然通常是推荐使用单向数据流动的，例如状态提升，[Flux](http://facebook.github.io/flux/)，或者 [Redux](http://redux.js.org/)，React 组件也可以使用一个其他框架和库的 Model 层。
...
负责渲染 model 的组件会监听 `'change'` 事件，而负责渲染 collection 的组件需要监听 `'add'` 和 `'remove'` 事件。在这两种情况中，调用 `this.forceUpdate()` （forceUpdate() ） 来使用新的数据重新渲染组件。
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/GmrREm?editors=0010)
...
[**在 CodePen 上运行**](https://codepen.io/gaearon/pen/PmWwwa?editors=0010)
