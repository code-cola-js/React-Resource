...
因为它的 value 只读，所以它是 React 中的一个**非受控**组件。将与其他非受控组件文件输入一起讨论。
...
另外，由于 `setState()` 自动将部分 state 合并到当前 state（见State 的更新会被合并）, 只需调用它更改部分 state 即可。

---

# 受控输入空值

在非受控组件上指定 `value` 的 prop 会阻止用户更改输入。如果你指定了 `value`，但输入仍可编辑，则可能是你意外地将 `value` 设置为 `undefined` 或 `null`。
