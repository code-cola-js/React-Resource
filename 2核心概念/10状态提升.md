...
我们知道Props 的只读性。当 `temperature` 存在于 `TemperatureInput` 组件的 state 中时，组件调用 `this.setState()` 便可修改它。然而，`temperature` 是由父组件传入的 prop，`TemperatureInput` 组件便失去了对它的控制权。
...
在 React 应用中，任何可变数据应当只有一个相对应的唯一“数据源”。通常，state 都是首先添加到需要渲染数据的组件中去。然后，如果其他组件也需要这个 state，那么你可以将它提升至这些组件的最近共同父组件中。你应当依靠数据是向下流动的，而不是尝试在不同组件间同步 state。
...
![](https://secure2.wostatic.cn/static/erwgkjoSmmRdAP3KwD3jqz/react-devtools-state.gif?auth_key=1668072305-fMbqP67SxjdYkm76zVSCi-0-3e03f5bbdc7f65ce7e3c9c4bf16788ad)