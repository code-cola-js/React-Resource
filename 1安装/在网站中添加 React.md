...
-   [一分钟用上 React](#wx4ZjAfuav44TQzJGk5Ui6)
-   [可选：使用 React 和 JSX](#7QP1ToGxXVrPSev1mPegPb) (不需要打包工具！)
...
这段代码定义了一个名为 `LikeButton` 的 React 组件。如果你还不明白这段代码的意思，不用担心 —— 我们将在后续的入门教程：认识 React和Hello World中介绍 React 的构建块。目前，我们先只做到显示！
...
当页面中以 React 驱动的不同部分是相互独立的时候，这种策略通常非常有用。在 React 代码中，使用组合组件（component composition） TODO 来实现会更容易。
...
```HTML
<script src="[https://unpkg.com/react@17/umd/react.production.min.js](https://unpkg.com/react@17/umd/react.production.min.js)" crossorigin></script>
<script src="[https://unpkg.com/react-dom@17/umd/react-dom.production.min.js](https://unpkg.com/react-dom@17/umd/react-dom.production.min.js)" crossorigin></script>
```
...
这两段代码是等价的。虽然JSX完全是可选的，但是多数人觉得这样编写 UI 代码更方便——无论是使用 React 还是其它库。

你可以通过 [在线编译器](https://babeljs.io/en/repl#?babili=false&browsers=&build=&builtIns=false&spec=false&loose=false&code_lz=DwIwrgLhD2B2AEcDCAbAlgYwNYF4DeAFAJTw4B88EAFmgM4B0tAphAMoQCGETBe86WJgBMAXJQBOYJvAC-RGWQBQ8FfAAyaQYuAB6cFDhkgA&debug=false&forceAllTransforms=false&shippedProposals=false&circleciRepo=&evaluate=false&fileSize=false&timeTravel=false&sourceType=module&lineWrap=true&presets=es2015,react,stage-2&prettier=false&targets=&version=7.15.7) 试用 JSX。
...
如果你看到一个错误消息显示为：“You have mistakenly installed the `babel` package”，你可能错过了[上一步](https://www.wolai.com/auSrxot44yh16MFocuwVa3#gfg6fqeLNbU8GdvWwbRphX)。在同一个文件夹中执行它，然后重试。
...
如果你认为你已经习惯了构建工具，并希望它们能为你做更多事，创建新的 React 应用描述了一些最流行和易上手的工具链。如果不使用构建工具 —— 直接以 scripts 标签的方式也可以很好地完成工作！